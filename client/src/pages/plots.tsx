...
function InteractiveHarvestTable({ plot, selectedCycle, harvestLogs }: InteractiveHarvestTableProps) {
  ...
  const InteractiveHarvestTable = ({ harvests, onEdit, onDelete }: any) => {
    ...
    const totalGradeA = harvests.reduce((sum: number, log: any) => sum + (Number(log.gradeAKg) || 0), 0);
    const totalGradeB = harvests.reduce((sum: number, log: any) => sum + (Number(log.gradeBKg) || 0), 0);
    const totalValueGradeA = harvests.reduce((sum: number, log: any) => {
      const gradeAKg = Number(log.gradeAKg) || 0;
      const priceGradeA = Number(log.pricePerKgGradeA || log.priceGradeA) || 0;
      return sum + (gradeAKg * priceGradeA);
    }, 0);
    const totalValueGradeB = harvests.reduce((sum: number, log: any) => {
      const gradeBKg = Number(log.gradeBKg) || 0;
      const priceGradeB = Number(log.pricePerKgGradeB || log.priceGradeB) || 0;
      return sum + (gradeBKg * priceGradeB);
    }, 0);
    const grandTotal = totalValueGradeA + totalValueGradeB;

    // Sort harvests by date (most recent first)
    ...
    return (
      <div className="space-y-4">
        <table className="min-w-full border border-gray-300 rounded-lg">
          ...
          <tbody>
            ...
            <tr className="bg-gray-100 dark:bg-gray-700 font-semibold">
              ...
              <td className="border border-gray-300 px-2 py-2 text-right">{formatMYR(totalValueGradeA)}</td>
              <td className="border border-gray-300 px-2 py-2 text-right">{formatMYR(totalValueGradeB)}</td>
              <td className="border border-gray-300 px-2 py-2 text-right text-lg">{formatMYR(grandTotal)}</td>
              <td className="border border-gray-300 px-2 py-2"></td>
            </tr>
          </tbody>
        </table>
        ...
        <div className="text-center space-y-2 text-sm text-gray-600 dark:text-gray-400">
          <div>Total harvest events: {sortedHarvests.length}</div>
